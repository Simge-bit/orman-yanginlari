...
def ingest_yillik_seri():
    df = pd.read_excel(data_path("raw", "ogm_yillik_yangin_1988_2025.xls"), header=None)
    rows = df.iloc[8:46]
    out = pd.DataFrame({
        "yil": rows[0],
...
def ingest_il_dagilim():
    df = pd.read_excel(data_path("raw", "ogm_il_yangin_dagilimi_2025.xlsx"), header=None)
    ...
    out.to_csv(data_path("interim", "il_dagilim_2025_ham.csv"), index=False)
    print(f"[ingest] il_dagilim_2025_ham.csv: {len(out)} satır (beklenen: 81)")


def ingest_orman_alani():
    df = pd.read_excel(data_path("raw", "ref_orman_alani_il_2025.xlsx"), header=None)
    rows = df.iloc[6:87]
    out = pd.DataFrame({
        "ibbs_kodu": rows[0],
        "il": rows[1],
...
def ingest_silvikultur_2025():
    # OGM Tablo 2.18: 2025'te yanan alana ne olduğu (ağaçlandırma programına
    ...
    df = pd.read_excel(
        data_path("raw", "ogm_silvikultur_degerlendirme_2025.xlsx"),
        sheet_name="2.18",
        skiprows=3,
        header=None,
        usecols=range(9),
    ...
    df.to_csv(data_path("interim", "silvikultur_2025_ham.csv"), index=False)
    print(f"[ingest] silvikultur_2025_ham.csv: {len(df)} satır (ulusal toplam dahil)")
...
def ingest_neden_bolge_2025():
    # OGM Tablo 2.15/2.16: 2025'te yangınların çıkış nedenini, ulusal
    ...
    alansal = _neden_bolge_tablosunu_oku("ogm_neden_bolge_alansal_2025.xlsx", "2.15", 5)
    sayisal = _neden_bolge_tablosunu_oku("ogm_neden_bolge_sayisal_2025.xlsx", "2.16", 4)
    alansal.to_csv(data_path("interim", "neden_bolge_alansal_2025_ham.csv"), index=False)
    sayisal.to_csv(data_path("interim", "neden_bolge_sayisal_2025_ham.csv"), index=False)
    print(f"[ingest] neden_bolge_alansal/sayisal_2025_ham.csv: {len(alansal)} satır (ulusal toplam dahil)")


def ingest_bolge_cok_yillik():
    # OGM Tablo 2.12 (alan) / 2.13 (sayı): bölge müdürlüğü bazında 2004-2025
    # arası 22 yıllık seri — daha önce "bölge verisi sadece 2025'te tek yıl
    # var" sanılıyordu, bu iki tablo aslında çok yıllı bir bölge trendi
    # hesaplamayı mümkün kılıyor. Geniş (yıl=sütun) formatta geliyor, uzun
    # (bolge, yil, deger) formata çevriliyor.
    yillar = list(range(2004, 2026))
    ...
    alansal = genisten_uzuna("ogm_bolge_alansal_2004_2025.xlsx", "2.12", "alan_ha")
    sayisal = genisten_uzuna("ogm_bolge_sayisal_2004_2025.xlsx", "2.13", "sayi")
...
def ingest_vasif_dagilimi_2025():
    # OGM Tablo 2.17: 2025'te yanan alan hangi orman türündeydi — sağlıklı/
    ...
    df = pd.read_excel(
        data_path("raw", "ogm_vasif_dagilimi_2025.xlsx"),
        sheet_name="2.17",
        header=None,
        skiprows=4,
        names=[
    ...
    df = df.dropna(subset=["bolge_muduru"])
    df = df[~df["bolge_muduru"].astype(str).str.startswith("Not")]
    df.to_csv(data_path("interim", "vasif_dagilimi_2025_ham.csv"), index=False)
    print(f"[ingest] vasif_dagilimi_2025_ham.csv: {len(df)} satır (ulusal toplam dahil)")
...
def main():
    ...
    ingest_silvikultur_2025()
    ingest_neden_bolge_2025()
    ingest_bolge_cok_yillik()
    ingest_vasif_dagilimi_2025()
